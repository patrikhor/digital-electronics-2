...
ISR(TIMER1_OVF_vect)
{
    ...
    // Start communication, transmit I2C Slave addDHTress, get result,
    ...
    //Measuring data from DHT12
    ack = twi_start(addDHT, TWI_WRITE);
    twi_write(0x00);
    twi_stop();

    ack = twi_start(addDHT, TWI_READ);
    ...
    ack = twi_start(addDS3231, TWI_WRITE);
    twi_write(0x00);
    twi_stop();

    ack = twi_start(addDS3231, TWI_READ);
